            ...
            productsCol.distinct('categoryInfo.cat_Level3',query,function(error,appMenuCatLevel3Docs) {
             ...
             if(error) {
              ...
              var pageCnt = Math.ceil(productsCnt / traPerPage);
                ...
                if(error) {
                  logError4(res,error,query,'EVM0016');
                } else {
                    res.render('motor/index-motor', { 
                                'user'            : req.user,
                                'langVar'         : language,
...
exports.appProductItem = function(db) {
 ...
 return function(req, res){
     var category = 'motor';
     var productsCol = db.get('products_motor');
     var appLanguageCol = db.get('app_text_language');
     ...
     if (req.params.lang == null)
      ...
      appLanguageCol.find({"language" : language},function(error,appMenuLangDocs){  
       if(error || appMenuLangDocs[0] == undefined) {
            logError3(res,error,'EVM0021');
        } else {
          productsCol.update({'_id' : req.params.id },{ $inc: {'advertisementInfo.adv_Viewers' : 1}},function(error,advViewsCntDocs){    
            if(error) {
              logError3(res,error,'EVM0022');
             } else {
               productsCol.find({'_id' : req.params.id},function(error,productsDocs){
                  if(error || productsDocs[0] == undefined) {
                   logError3(res,error,'EVM0023');
                  } else {
                     var appMenuCol = db.get('app-text-motor-typ');
                     appMenuCol.find({"motor" : productsDocs[0].categoryInfo.cat_Level1,"model" : productsDocs[0].categoryInfo.cat_Level2, "typ" : productsDocs[0].categoryInfo.cat_Level3},function(e,appMenuDocs){
                     res.render('motor/product-item-motor', { 
                                'user'            : req.user,
                                'langVar'         : language,
                                'categoryVar'     : category,
                                'searchVar'       : req.params.searchView,
...
exports.appProductSearch  = function() {
  return function(req, res){
    ...
    res.redirect('/index-motor/'+req.params.lang+'/1/0/0/'+cat_Level1+'/'+cat_Level2+'/'+cat_Level3+'/'+det_Body+'/'+det_Transmition+'/'+det_Color+'/'+det_Fuel+'/'+det_MinYear+'/'+det_MaxYear+'/'+det_MinEngine+'/'+det_MaxEngine+'/'+det_MinMileage+'/'+det_MaxMileage+'/'+det_MinPrize+'/'+det_MaxPrize+'/'+con_City+'/'+con_Country +'?menu=' +req.query.menu );
  }
}
